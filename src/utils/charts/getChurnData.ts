...
const getChurnChartData = ({
  deletedContracts,
  initialDate,
  finalDate,
}: IParams): IChurnChartData[] => {
  ...
  for (const deletedContract of deletedContracts) {
    if (deletedContract.deletedAt) {
      ...
      if (
        contractDeleteData >= initialDate &&
        contractDeleteData <= finalDate
      ) {
        let dataFound = false;
        const contractKey = `${
          contractDeleteData.getMonth() + 1
        }-${contractDeleteData.getFullYear()}`;
        ...
        const deleteMonth = contractDeleteData.toLocaleString("pt-BR", {
          month: "long",
          year: "numeric",
        });
